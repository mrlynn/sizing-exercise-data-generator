...
var insertDocuments = function (db, callback) {
	...
	for (var i = 0; i < process.env.HOWMANY; i++) {
		...
		surveyDocument = {
			...
			"remind_date": new Date(faker.date.past()),
			"survey_end_date": new Date(faker.date.past()),
			"firstname": firstName,
			"crcreceiveddate": new Date(faker.date.past()),
			...
			"surveytype": "BIG5",
			"custfname": firstName,
			"crcdate": new Date(faker.date.past()),
			"fiosrepairdisp": "Y",
			"phone": phone,
			...
			"servicetype": "repair",
			"techarrivaldate": new Date(faker.date.past()),
			"crctypeoftrbl": "9",
			...
			"recv_dt": new Date(faker.date.past()),
			"email_open_count": 0,
			"selfinstallreport": "0",
			"wire_center": state + "2238",
			"crcnoofsubrpts": "0",
			"to_address": email,
			"samplingloaddate": new Date(faker.date.past()),
			...
			"responseDate": new Date(faker.date.past()),
			"response_date": new Date(faker.date.past()),
